module.exports = ({ code, minutes = 5, purpose = 'login' }) => {
  const label = purpose === 'signup' ? 'Sign Up' : 'Login';
  const subject = `Your OTP for ${label}: ${code}`;
  const text = `Your OTP code is ${code}. It expires in ${minutes} minutes.`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
      <h2 style="color:#222;">Himashi Properties ${label}</h2>
